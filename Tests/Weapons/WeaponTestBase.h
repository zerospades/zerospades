...
namespace spades {
	namespace tests {
		...
		// because Tests/Physics is not on the Tests/Weapons include path (only
		// Tests/Helpers + Sources are). Floats use EXPECT_NEAR with the per-field
		// tolerance (ToleranceForField on the dotted path); everything else is an
		// exact EXPECT_EQ. Never uses the banned EXPECT_FLOAT_EQ. Flags both
		// missing and extra keys (symmetric).

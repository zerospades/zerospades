...
namespace spades {
	namespace draw {
		...
		void VulkanFlatMapRenderer::UpdateChunks() {
			// Each dirty chunk costs a staging buffer, a one-shot command buffer and a
			// queue stall, so the common case -- nothing was dug or built this frame --
			// must not even scan the chunk list.
			if (!hasDirtyChunks)
				return;

...
namespace spades {
	namespace gui {
		...
		bool KV6IsEditable(const std::string& name) { return ToLowerExt(name, 4) == ".kv6"; }
		...
		KV6ListItem::KV6ListItem(UIManager* manager, KV6Entry entry)
		    : ui::ButtonBase(manager), entry(std::move(entry)) {}

		void KV6ListItem::Render() {
			...
			if (entry.isFolder) {
				...
				label = entry.name + "/";
			} else if (KV6IsEditable(entry.name)) {
		...
		KV6ListModel::KV6ListModel(UIManager* manager, std::vector<KV6Entry> entries)
		    : manager(manager), entries(std::move(entries)) {
			itemElements.resize(this->entries.size());
		}

		void KV6ListModel::OnItemClicked(UIElement& sender) {
			KV6ListItem* item = dynamic_cast<KV6ListItem*>(&sender);
			if (item && itemActivated)
				itemActivated(item->GetEntry().name, item->GetEntry().isFolder);
		}

		void KV6ListModel::OnItemDoubleClicked(UIElement& sender) {
			KV6ListItem* item = dynamic_cast<KV6ListItem*>(&sender);
			if (item && itemDoubleClicked)
				itemDoubleClicked(item->GetEntry().name, item->GetEntry().isFolder);
		}

		Handle<UIElement> KV6ListModel::CreateElement(int row) {
			if (!itemElements[row]) {
				Handle<KV6ListItem> i = Handle<KV6ListItem>::New(manager, entries[row]);
